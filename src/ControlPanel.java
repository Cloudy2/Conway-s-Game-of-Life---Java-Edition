...
public class ControlPanel extends JPanel implements MouseListener, ActionListener {
   ...
   public ControlPanel(World world) {
      this.SIDE_LENGTH = App.SIDE_LENGTH;
      this.buttons = new ArrayList<NamedButton>();
      this.flowLayout = new FlowLayout();
      this.world = world;
      int controlWidth = 500 / this.SIDE_LENGTH * this.SIDE_LENGTH;
      this.setPreferredSize(new Dimension(controlWidth, 100));
      this.setSize(controlWidth, 100);
      this.setLayout(this.flowLayout);
      this.setBackground(Color.gray);
      this.buttonWidth = this.getWidth() / 5;
      this.buttonHeight = this.getHeight() / 4;
      this.createNewButton("Play");
      this.createNewButton("Next");
      this.createNewButton("Reset");
      this.createNewButton("Import");
   }
   ...
   private void convertFileToImage(File file) {
      try {
         ...
         if (image.getWidth() == this.SIDE_LENGTH && image.getHeight() == this.SIDE_LENGTH) {
            this.world.mapImageToWorld(image);
         } else {
            System.out.println("Image not the same size as the world!");
         }
      } catch (IOException var3) {
         System.err.println("Error reading the image file: " + var3.getMessage());
         var3.printStackTrace();
      } catch (IllegalArgumentException var4) {
         System.err.println("File is null or does not exist: " + var4.getMessage());
      }
